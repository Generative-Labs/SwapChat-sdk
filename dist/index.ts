(function (global, factory) {
  ...
  const getElementById = (className = "") => {
  ...
  const baseUrl = "https://chat.web3messaging.online";
  ...
  const axiosApiInstance = axios__default["default"].create();
  ...
  axiosApiInstance.interceptors.request.use(async config => {
    ...
    if (accessToken) {
      ...
      if (moment.utc().isAfter(moment(accessExpiredAt).utc())) {
        ...
        try {
          token = await axios__default["default"].get(refreshAPIUrl, {
            headers: {
  ...
  async function getOpenSeaInfo$1(params) {
    return await axiosApiInstance.post("/opensea_user_info", params);
  }
  async function getLoginRandomSecret(params) {
  ...
  async function getEthAccount() {
    ...
    if (!isSafePal) {
      // @ts-ignore
      const permissionRes = await web3.currentProvider // @ts-ignore
      .request({
  ...
  const loginAfterSign = async (res = "", address = "", signContent = "", userAvatar = "") => {
    let loginParams = {
  ...
  const signMetamask = async (platform = PLATFORM_ENUM.SWAPCHAT) => {
    let targetToken = ''; // @ts-ignore
    ...
    let randomSecret = await getLoginRandomSecret({
      wallet_address: address
    });
    ...
    if (randomSecret.code !== 0) {
      ...
      randomSecret = await getLoginRandomSecret({
        wallet_address: address
      });
    }
...
Issued At: ${moment().utc().local().format("DD/MM/YYYY HH:mm")}`;
      const res = await web3.eth.personal.sign(signContent, address, "swapchat").catch(e => {});
  ...
  const register = async params => {
    const Info = await register$1({ ...params
    });
    const {
      code,
      data: {
  ...
  const getOpenSeaInfo = async params => {
    const openseaInfo = await getOpenSeaInfo$1({ ...params
    });
    const {
      code,
      data: {
  ...
  const creactThreads = async params => {
    const createThreadsRes = await createThreads({ ...params
    });
    const {
      code,
      data: {
  ...
  class SwapChatSdk {
    ...
    async creactIframeUrl() {
      ...
      if (!isFreshToken(targetToken)) {
        if (login_paylod && login_random_secret && signature && login_paylod.wallet_address && login_paylod.user_avatar) {
          targetToken = await loginAfterSign(signature, login_paylod.wallet_address, login_random_secret, login_paylod.user_avatar);
        } else {
      ...
      async function getRoomIdByParams(params) {
        ...
        const roomData = await getRooms({ ...params
        });
      ...
      try {
        if (targetToken) {
          targetToken = targetToken.substring(7);
          iframeUrl += `&accessToken=${encodeURIComponent(targetToken)}`;
        }

        if (targetRoomId) {
          iframeUrl += `&roomId=${encodeURIComponent(targetRoomId)}`;
        } else {
    ...
    async creatClientBox() {
      ...
      let messageBoxEle = createElement(`<div id="web3-swapchat-message-box" style="min-width:120px;width:100%;width:${this.defaultOptions.width}px;overflow:auto">
            </div>`);
      let homeIconEle = createElement("<img class='home-icon' src='https://chat.web3messaging.online/assets/icon/newHomeHeaderIcon.svg' alt='' style='width: auto;height: 28px;margin-right: 8px;border-radius: 4px;' >");
      let goHomeIconEle = createElement("<img class='go-home-icon' src='https://d97ch61yqe5j6.cloudfront.net/frontend/newRefreshIconx3.png' alt='' style='position: absolute;right: 50px;height: 22px;width: auto;z-index: 999;margin-right: 8px;border-radius: 4px; cursor: pointer;' >");
      let slideToggleIconELe = createElement("<img class='slide-toggle-icon' id='web3-slide-toggle-icon'  src='https://d97ch61yqe5j6.cloudfront.net/frontend/headerUp.png' alt='' style='position: absolute;right: 12px;height: 22px;width: auto;z-index: 999;margin-right: 8px;border-radius: 4px;cursor: pointer;'>");
      let messageHeaderEle = createElement(`<div class="twitter-swapchat-message-header" style='min-width:240px;position:relative;width:100%;align-items: center;
      width: 100%;
      display:flex;
      height: 50px;
      line-height: 50px;
      ...
      border-bottom: 1px solid #F2F2F2;' >
        ...
        `);
      let messageBodyEle = createElement("<div class='twitter-swapchat-message-body' id ='twitter-swapchat-message-body' style='overflow:hidden;transition: max-height .25s;max-height:1000px'></div>");
